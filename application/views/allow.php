...
<?php 
	foreach ($chats as $chat) {
		...
		?>
	  <div class="row">
	  	<div class="col-sm-1"><img src="<?= $chat['type']==CHAT_TYPE_PRIVATE?asset_base_url().'/images/ava-group.svg':asset_base_url().'/images/ava-single.svg'?>" width="38"></div>
	    <div class="col-sm-2"><span><?= $chat['name']?></span></div>
	    <div class="col-sm-5"><a href="<?= site_url('chat/channel/'.$chat['did'])?>"><?= str_replace(array("[","]","\""), "", $emails)?></a></div>
...
<?php if ($chat['status'] == 0) {?>
	    ...
	    <div class="col-sm-1"><a class="btn btn-success" href="<?= site_url('allow')?>/action/<?= $chat['did']."/".$page?>">Active</a></div>
<?php } else if ($chat['status'] == 1) {?>
		<div class="col-sm-1"><span class="text-success">Activated</span></div>
	    <div class="col-sm-1"><a class="btn btn-warning" href="<?= site_url('allow')?>/action/<?= $chat['did']."/".$page?>">Deactive</a></div>
<?php }?>
		<div class="col-sm-1"><a class="btn btn-danger" onclick="delAction(this, '<?= $chat['did']?>', '<?= $chat['name']?>')" data-act="<?= site_url('allow')?>/delete/<?= $chat['did']."/".$page?>">Delete</a></div>
	  </div>
<?php		
	}
?>
